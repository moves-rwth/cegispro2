...
def Fair(inpt, task, NUM_runs, assumption):
    ...
    for _ in range(NUM_runs):
        ...
        # start the loop body
        c1 = bernoulli.rvs(size=1, p=0.5)[0]
        if c1:
            count = count + 1
        c2 = bernoulli.rvs(size=1, p=0.5)[0]
        if c2:
            count = count + 1
        # end the loop body
        scope = locals()
...
def Mart(inpt, task, NUM_runs, assumption):
    ...
    post_states_lst = []
    for _ in range(NUM_runs):
        rounds, b = inpt["rounds"], inpt["b"]
        # start the loop body
        d = bernoulli.rvs(size=1, p=0.5)
        if d:
            b = 0
        else:
            b = 2 * b
...
def Bin0(inpt, task, NUM_runs, assumption):
    ...
    # sampling
    for _ in range(NUM_runs):
        n, x, y = inpt["n"], inpt["x"], inpt["y"]
        # start of the loop body
        d = bernoulli.rvs(p=0.5, size=1)[0]
        if d:
            x = x + y
        n = n - 1
        # end of the loop body
...
def BiasDir(inpt, task, NUM_runs, assumption):
    ...
    for _ in range(NUM_runs):
        x, y = inpt["x"], inpt["y"]
        d1 = bernoulli.rvs(size=1, p=0.5)[0]
        if d1:
            x = 1
        else:
            x = 0
        d2 = bernoulli.rvs(size=1, p=0.5)[0]
        if d2:
            y = 1
        else:
            y = 0
...
def Duel(inpt, task, NUM_runs, assumption):
    ...
    for _ in range(NUM_runs):
        ...
        if t == 1:
            d1 = bernoulli.rvs(size=1, p=0.4)[0]
            if d1:
                c = 0
            else:
                t = 0
        else:
            d2 = bernoulli.rvs(size=1, p=0.6)[0]
            if d2:
                c = 0
            else:
                t = 1
